...
split {
	switch ((short)vars.game) {
		...
		case 7:
			...
			var splitSeed = (Func <float, float, float, bool>)((xTarg, yTarg, zTarg) => { return vars.inPosWithRange(xTarg, yTarg, zTarg, 3) && seedGet ? true : false; });
			
			//This function checks if x,y,z co-ordinates are in a certain range and if a combat has just ended:
			var splitBoss = (Func <float, float, float, bool>)((xTarg, yTarg, zTarg) => { return vars.inPosWithRange(xTarg, yTarg, zTarg, 3) && kill ? true : false; });

			// Initializing 2k8 Splits:
			bool Alchemist        = splitBoss(-296.593f, 697.233f, 296.199f) ? true : false;
			...
			bool Cauldron         = splitSeed(107.123f, 183.394f, -5.628f) ? true : false;
			bool Cavern           = splitSeed(251.741f, 65.773f, -13.616f) ? true : false;
			bool CityGate         = splitSeed(547.488f, 45.41f, -27.107f) ? true : false;
			bool Concubine        = splitBoss(352.792f, 801.051f, 150.260f) ? true : false;
			bool ConstructionYard = splitSeed(-151.121f, 303.514f, 27.95f) ? true : false;
			bool CoronationHall   = splitSeed(264.497f, 589.336f, 38.67f) ? true : false;
			bool CoronationHallH  = splitBoss(340f, 582.5f, 32.5f) ? true : false;
			...
			bool HeavensStair     = splitSeed(-85.968f, 573.338f, 30.558f) ? true : false;
			bool HeavensStairH    = splitBoss(-291f, 651.5f, 99.2f) ? true : false;
			bool Hunter           = splitBoss(-929.415f, 320.888f, -89.038f) ? true : false;
			bool King             = splitBoss(5f, -365f, -32f) ? true : false;
			bool KingsGate        = splitSeed(-538.834f, -67.159f, 12.732f) ? true : false;
			bool MachineryGround  = splitSeed(-361.121f, 480.114f, 12.928f) ? true : false;
			bool MarshGrounds     = splitSeed(-806.671f, 112.803f, 21.645f) ? true : false;
			bool MartyrsTower     = splitSeed(-564.202f, 207.312f, 22f) ? true : false;
			bool MTtoMG           = splitSeed(-454.824f, 398.571f, 27.028f) ? true : false;
			bool QueensTower      = splitSeed(637.262f, 27.224f, -28.603f) ? true : false;
			bool Reservoir08      = splitSeed(-150.082f, 406.606f, 34.673f) ? true : false;
			bool Resurrection     = vars.inXRange(5.562f, 5.566f) && vars.inYRange(-222.745f, -222.517f) && current.zPos >= -33.1 ? true : false;
			bool SpireOfDreams    = splitSeed(-28.088f, 544.298f, 34.942f) ? true : false;
			bool SunTemple08      = splitSeed(-670.471f, -56.147f, 16.46f) ? true : false;
			...
			bool TowerOfOrmazd    = splitSeed(609.907f, 61.905f, -35.001f) ? true : false;
			bool Warrior          = splitBoss(1070.478f, 279.147f, -29.571f) ? true : false;
			bool Windmills        = splitSeed(-597.945f, 209.241f, 23.339f) ? true : false;
		...
		case 8:
			...
			//This function checks if x,y,z co-ordinates are in a certain range and if a checkpoint has just been acquired:
			var splitCP = (Func <float, float, float, bool>)((xTarg, yTarg, zTarg) => { return vars.inPosWithRange(xTarg, yTarg, zTarg, 10) && cpGet ? true : false; });
			
			//This function checks if x,y,z co-ordinates are within range of 1 unit from the target split location:
			var splitXYZ = (Func <float, float, float, bool>)((xTarg, yTarg, zTarg) => { return vars.inPosWithRange(xTarg, yTarg, zTarg, 1) ? true : false; });

			// Initializing TFS Splits:
			bool Climb            = splitCP(912, 256, -56) ? true : false;
			bool CourtyardTFS     = splitCP(-434, -533, -127) ? true : false;
			bool TFSEnd           = splitXYZ(821, -257, -51) ? true : false;
			bool Gardens          = splitCP(240, -227, -114) ? true : false;
			bool Malik            = splitCP(-37, 231, -148) ? true : false;
			bool ObservatoryTFS   = splitCP(-510, 460, 104) ? true : false;
			bool Possession       = splitXYZ(89, -477, -83) && cpGet ? true : false;
			bool PowerOfKnowledge = splitCP(548, -217, 4) ? true : false;
			bool PowerOfLight     = splitCP(540, -219, 6) ? true : false;
			bool PowerOfRazia     = splitCP(430, 268, -99) ? true : false;
			bool PowerOfTime      = splitCP(597, -217, -2) ? true : false;
			bool PowerOfWater     = splitCP(519, -227, 6) ? true : false;
			bool Ratash           = splitCP(-406, 403, 64) ? true : false;
			bool ReservoirTFS     = splitCP(644, 385, -63) ? true : false;
			bool SewersTFS        = splitCP(-228, 245, 20) ? true : false;
			bool Storm            = splitCP(948, -284, 86) ? true : false;
			bool Works            = splitCP(-513, -408, -167) ? true : false;
